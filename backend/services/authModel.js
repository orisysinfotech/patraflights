...
export const requestOtp = async ({ driverId, mobileNumber }) => {
  ...
  await otpService.sendSMS(mobileNumber, otpCode);
...
export const verifyOtp = async ({ mobileNumber, otp, ip }) => {
  if (!mobileNumber || !otp) {
  ...
  const [existingDrivers] = await db.execute(
    "SELECT driver_id, driver_regno, otp_code, otp_expires_at, driver_fstname, is_mpin_set FROM tbl_drivers WHERE driver_phno = ?",
    [mobileNumber]
  );
  ...
  if (!isValidOtp || !isNotExpired) {
    ...
    console.log(`Auth Failed for ${mobileNumber}: Match=${isValidOtp}, NotExpired=${isNotExpired}`);
...
export const updateDriverprofile = async (driverId, data, photoPath) => {
  const { address1, address2, address3, state_id, city_id, zipcode, utype } = data;
  ...
  try {
    ...
    if (existing.length > 0) {
      ...
      await connection.execute(
        ...
        [state_id, city_id, zipcode, address1, address2, address3, driverId, utype, driverId]
      );
    } else {
      ...
      await connection.execute(
        `INSERT INTO tbl_addresses 
         ...
         VALUES (?, ?, ?, ?, '99', ?, ?, ?, ?, NOW(), ?)`,
        [utype, driverId, state_id, city_id, zipcode, address1, address2, address3, driverId]
      );
    }
...
export const requestPhoneChangeOtp = async (driverId, { newMobileNumber }) => {
  ...
  await otpService.sendSMS(newMobileNumber, otpCode);
...
export const updateDriverDocuments = async (driverId, euType, body, files) => {
  ...
  try {
    ...
    for (const param of params) {
      ...
      // Extract values and ensure they are either the value or null (not undefined)
      const idntNo =
        body[`${fieldPrefix}_no`] !== undefined
          ? body[`${fieldPrefix}_no`]
          : null;
      const fileObj =
        files && files[`${fieldPrefix}_file`]
          ? files[`${fieldPrefix}_file`][0]
          : null;
      const expiryDate = fieldPrefix === "dl" ? body.dl_expiry || null : null;

      // ONLY PROCESS IF: There is a new number OR a new file
      // This makes the fields non-mandatory
      if (idntNo !== null || fileObj !== null) {
        ...
        if (existing.length > 0) {
          // UPDATE: Preserve old data if new data is not provided
          const finalNo = idntNo !== null ? idntNo : existing[0].idnt_no;
          const finalFile =
            fileName !== null ? fileName : existing[0].idnt_copy;
          const finalExpiry =
            expiryDate !== null ? expiryDate : existing[0].idnt_expdt;
          ...
          // INSERT: Only if at least one piece of data exists for this doc type
          await connection.execute(
    ...
    await connection.commit();
    return { message: "Documents synchronized successfully." };
  } catch (err) {
    if (connection) await connection.rollback();
    throw err;
  } finally {
